...
class LaravelGenerator:
    ...
    def processfile(self):
        ...
        if os.path.exists(self.filename):  
            ...
            columns, fillable_attributes = self.extract_top_level_keys_with_types(data)

            table_name = os.path.splitext(os.path.basename(self.filename))[0].replace("-", "_")
                ...
                if isinstance(item, dict):
                    for key, value in item.items():
                        if key not in seen_keys:  
                            laravel_type = self.map_type_to_laravel(type(value).__name__)
                            columns.append(f"            $table->{laravel_type}('{key}');")
                            fillable_attributes.append(f"'{key}'")
                            seen_keys.add(key)  
        elif isinstance(json_data, list) and json_data:
            return self.extract_top_level_keys_with_types(json_data[0])  

        return columns, fillable_attributes

    def map_type_to_laravel(self,python_type):
    ...
    def write_laravel_migration(self,table_name, columns):
    ...
    def write_laravel_model(self,table_name, fillable_attributes):
    ...
    def write_laravel_controller(self,table_name, fillable_attributes):
...
for i in range(startfromfile, endatfile + 1):
    filename = f"file_{i}d.json"
    # filename = "file_2.json"
    try:
        laravel_generator = LaravelGenerator(filename)
    except:
        print("error during class runinig")